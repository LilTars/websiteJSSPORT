...
function TopBar({ utilityItems: items, theme, onToggleTheme }: TopBarProps) {
    return (
        <div className="border-b border-gray-200 bg-gray-100 dark:border-slate-800 dark:bg-slate-950">
            <div className="mx-auto flex h-11 w-full max-w-7xl items-center justify-between px-4 md:px-6 lg:px-8">
                ...
                <div className="flex items-center gap-3 text-xs text-gray-700 dark:text-slate-300">
                    <button
                        ...
                        className="inline-flex items-center gap-1.5 rounded-full border border-gray-300 bg-white px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.12em] text-gray-700 transition hover:border-pink-400 hover:text-pink-600 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-pink-500 dark:hover:text-pink-400"
    ...
    return (
        <div className="border-b border-gray-200 bg-white dark:border-slate-800 dark:bg-slate-950">
            <div className="mx-auto flex h-20 w-full max-w-7xl items-center justify-between px-4 md:px-6 lg:px-8">
                <Link href="/" className="inline-flex w-56 items-center justify-start">
                    <img
                        src="/images/logos/logojs.png"
                        alt="Main logo"
                        className="h-35 w-auto object-contain transition-all duration-300 ease-in-out hover:scale-105"
                    />
                </Link>
                ...
                <button
                    type="button"
                    className="inline-flex items-center justify-center rounded-md border border-gray-300 p-2 text-gray-900 transition-all duration-300 ease-in-out hover:border-pink-400 hover:bg-pink-50 hover:text-pink-600 dark:border-slate-700 dark:text-slate-100 dark:hover:bg-slate-900 dark:hover:text-pink-400 md:hidden"
...
function PromoBanner() {
    return (
        <div className="border-b border-gray-200 bg-white dark:border-slate-800 dark:bg-slate-950">
            <p className="px-4 py-2 text-center text-xs font-medium text-gray-700 dark:text-slate-300 md:text-sm">
                ส่งฟรีเมื่อสั่งซื้อครบ ฿1,500 และรับสิทธิ์พิเศษก่อนใครสำหรับสมาชิก
            </p>
        </div>
    );
}

function MobileMenu({ isOpen, currentPath, menuItems: items, onClose }: MobileMenuProps) {
    return (
        <div
                ...
                <nav className="flex flex-col gap-2">
                    {items.map((item) => (
                        <Link
                            ...
                            href={item.href}
                            onClick={onClose}
                            className={`rounded-md px-3 py-2 text-sm font-semibold transition-all duration-300 ease-in-out ${
                                isPathActive(currentPath, item.href)
